...
//    this.setErrorString();
  ...
  //for tables like ILIAS
  nextId: function(sObjName) {
	var iValue = 1;
	var sObjName_id = sObjName+"_id";
	if (sObjName == "cmi_correct_response") sObjName_id = "cmi_correct_resp_id";
	sObjName = this.getPrefixedName(sObjName, "");

	var sQuery = "SELECT max("+sObjName_id+") FROM " + sObjName;
	this.selectQuery(sQuery);

	if (this.aTableData[0][0] != null) {
		iValue = this.aTableData[0][0] + 1;
	}
	return iValue;
  },
	...
	try {
		for (var i = 0; i < aParamData.length; i++) {
			...
			switch (pType) {
				...
				case "integer":
				case SQLiteTypes.INTEGER:
					if (typeof aData[1] == "undefined" || aData[1] == null) stmt.bindNullParameter(aData[0]);
					else stmt.bindInt64Parameter(aData[0], aData[1]);
					break;
				case "real":
				case SQLiteTypes.REAL:
					if (typeof aData[1] == "undefined" || aData[1] == null) stmt.bindNullParameter(aData[0]);
					else stmt.bindDoubleParameter(aData[0], aData[1]);
					break;
				case "text":
				case SQLiteTypes.TEXT:
					if (typeof aData[1] == "undefined" || aData[1] == null) stmt.bindNullParameter(aData[0]);
					else stmt.bindStringParameter(aData[0], aData[1]);
					break;
				case "blob":
				case SQLiteTypes.BLOB:
				if (typeof aData[1] == "string") aData[1] = this.textToBlob(aData[1]);
					if (typeof aData[1] == "undefined" || aData[1] == null) stmt.bindNullParameter(aData[0]);
					else stmt.bindBlobParameter(aData[0], aData[1], aData[1].length);
					break;
			}
		}
	} catch (e) {
