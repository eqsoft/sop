...
function validateLm(lm) {
	...
	if (lm.description===null || undefined===lm.description || lm.description=="") {
		//utils.log("invalid status: " + lm.status + ". Set to 0=not_attempted");
		lm.description = "no description";
	}
	return lm;
}
...
//	var r = db.getData("lmGetAllCmiNodeByClientAndObjId",[client,obj_id],true,true);
	var req = Request({
		...
		onComplete: function (response) {
			...
			var r = db.setData("lmInactive",[client,obj_id]);
			if (r==true) utils.log("sop2il successful");
			getAllLm(false);
			//delete  sahs_user,lm
			//r = db.setData("il2sopCmiDelete",[client,obj_id]);
