...
function getOfflineUrl(id) {
	...
	var domain = (!lm.domain) ? "" : atoB(lm.domain);
	var url = env.globals.serverProtocol + "://" + env.globals.serverName + ":" + env.globals.serverPort + "/" + player + "?" + "client=" + lm.client + "&obj_id=" + lm.obj_id + "&domain=" + domain;
	return url;
}
