...
function dbData(statement, params) {
	...
	switch (statement) { // ToDo: define propper views and params in sqlite and get data with getView(view,params)
		case "lmGetAll" : 
			ret = dbHandler.manipulateF("SELECT lm.domain, lm.client, lm.obj_id, "
		...
		case "lmGetAllByClientAndObjId" :
			ret = dbHandler.manipulateF(
				"SELECT lm.domain, lm.client, lm.obj_id, "
				+"lm.title, lm.description, lm.scorm_version, lm.active, lm.offline_zip_created, "
				+"sahs_user.package_attempts, sahs_user.last_access, sahs_user.total_time_sec, "
				+"sahs_user.sco_total_time_sec, sahs_user.status, sahs_user.percentage_completed "
				+"from lm,sahs_user "
		...
		case "lmSetStatusByClientAndObjId" :
			ret = dbHandler.manipulateF(
				"UPDATE sahs_user set status=%s WHERE client=%s AND obj_id=%s",
				new Array("integer","text","integer"),
