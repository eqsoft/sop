	...
	switch (statement) { // ToDo: define propper views and params in sqlite and get data with getView(view,params)
		case "lmGetAll" : 
			ret = dbHandler.manipulateF("SELECT lm.client, lm.obj_id, "
				+"lm.title, lm.description, lm.scorm_version, lm.active, lm.offline_zip_created, "
				+"sahs_user.package_attempts, sahs_user.first_access, sahs_user.last_access, sahs_user.total_time_sec, "
		...
		case "lmGetAllByClientAndObjIdAtInitOfPlayer" :
			ret = dbHandler.manipulateF(
				"SELECT package_attempts,first_access FROM sahs_user WHERE client=%s AND obj_id=%s",
				new Array("text","integer"),
				new Array(params[0],params[1])
				);
			...
			ret = dbHandler.manipulateF(
				"UPDATE sahs_user set package_attempts=%s, first_access=%s WHERE client=%s AND obj_id=%s",
				new Array("integer","integer","text","integer"),
				new Array(package_attempts,first_access,params[0],params[1])
				);
			ret = dbHandler.manipulateF(
				"SELECT init_data, resources, scorm_tree, sahs_user.module_version, user_data, last_visited, sahs_user.status FROM lm, sahs_user "
				+"WHERE lm.client=sahs_user.client AND lm.obj_id=sahs_user.obj_id AND lm.client=%s AND lm.obj_id=%s",
				new Array("text","integer"),
				new Array(params[0],params[1])
				);
...
//check
//ret = dbHandler.manipulateF("SELECT count(*) FROM cmi_comment WHERE cmi_node_id = %s",new Array("INTEGER"), new Array($cmi_node_id));
ret = dbHandler.getRowCount("cmi_comment","WHERE cmi_node_id = "+$cmi_node_id);
utils.log("setCMIData, cmi_node: "+ $cmi_node_id+ ", cmi_comment returns count: "+dbHandler.getRecords())
						//to send to client
...
//org							$a_map_cmi_interaction_id[]=new Array($row[0],$cmi_interaction_id);
...
// //		}
		...
		if ($data.lp_mode=="12") //12=scorm_package - has to be checked UK
		{
...
*/
			...
			var last_access = d_now.getTime();
			var percentage_completed = null;
			if (lp_mode == 6 && typeof $data.percentageCompleted == "number") percentage_completed = Math.round($data.percentageCompleted);
			var sco_total_time_sec = null;
		...
		case "il2sopLm" :
			...
			if(counter==0) {
				ret = dbHandler.manipulateF(
					"INSERT INTO lm (title, description, scorm_version, active, init_data, resources, scorm_tree, module_version, offline_zip_created, learning_progress_enabled, certificate_enabled, max_attempt, client, obj_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) ",
					new Array("TEXT","TEXT","TEXT","INTEGER","TEXT","TEXT","TEXT","INTEGER","INTEGER","INTEGER","INTEGER","INTEGER","TEXT","INTEGER"), params);
			} else {
				ret = dbHandler.manipulateF(
					"UPDATE lm SET title=%s, description=%s, scorm_version=%s, active=%s, init_data=%s, resources=%s, scorm_tree=%s, module_version=%s, offline_zip_created=%s, learning_progress_enabled=%s, certificate_enabled=%s, max_attempt=%s WHERE client=%s AND obj_id=%s",
					new Array("TEXT","TEXT","TEXT","INTEGER","TEXT","TEXT","TEXT","INTEGER","INTEGER","INTEGER","INTEGER","INTEGER","TEXT","INTEGER"), params);
			}
			break;
		case "il2sopSahsUser" :
