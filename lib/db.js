	...
	switch (statement) { // ToDo: define propper views and params in sqlite and get data with getView(view,params)
		case "lmGetAll" : 
			ret = dbHandler.manipulateF("SELECT lm.client, lm.obj_id, "
				...
				+"sahs_user.package_attempts, sahs_user.last_access, sahs_user.total_time_sec, "
				+"sahs_user.sco_total_time_sec, sahs_user.status, sahs_user.percentage_completed "
				+"from lm,sahs_user "
				+"WHERE lm.client=sahs_user.client AND lm.obj_id=sahs_user.obj_id");
			break;
		...
		case "lmGetAllByClientAndObjIdAtInitOfPlayer" :
			ret = dbHandler.manipulateF(
				"SELECT package_attempts FROM sahs_user WHERE client=%s AND obj_id=%s",
				new Array("text","integer"),
				new Array(params[0],params[1])
				);
			var package_attempts = parseInt(dbHandler.getRecords())+1;
			ret = dbHandler.manipulateF(
				"UPDATE sahs_user set package_attempts=%s WHERE client=%s AND obj_id=%s",
				new Array("integer","text","integer"),
				new Array(package_attempts,params[0],params[1])
				);
			ret = dbHandler.manipulateF(
				"SELECT init_data, resources, scorm_tree, sahs_user.module_version, user_data, last_visited FROM lm, sahs_user "
				+"WHERE lm.client=sahs_user.client AND lm.obj_id=sahs_user.obj_id AND lm.client=%s AND lm.obj_id=%s",
				new Array("text","integer"),
				new Array(params[0],params[1])
				);
