...
/**
 * EventListener for install event, will do a startup (ToDo: db und data updates must be triggered) 
 * @private
 * @method install
 **/
function install() {
...
function renderLm(id,str=false) {
	...
	var dl2 = new Date(lm.first_access);
	
	var firstaccess = dl2.toLocaleString();
