...
var mwin = null; // manager window (only one instance)
		...
		default :
			...
			for (var i=0; i<metas.length; i++) { 
				if (metas[i].getAttribute("name") == "require-sop-version") {
					...
					w.sopConnector = Cu.cloneInto(som.connectorFuncs,w,{cloneFunctions: true});
				} 
			}
...
function initManager(tab) {
	...
	try {
		...
		mwin = (mwin != null) ? mwin : winutils.getMostRecentBrowserWindow();
		/*
		if (winutils.isDocumentLoaded(mwin.content)) { // bug in mobile fennec (cache??)
			utils.log("mwin content is already loaded?");
			//onManagerLoad();
			//onPrefStopSom();
			//onPrefStartSom();
			return;
		...
		*/ 
		...
		mw.som = Cu.cloneInto(som,mw,{cloneFunctions: true});
		mw.db = Cu.cloneInto(db,mw,{cloneFunctions: true});
		mw.utils = Cu.cloneInto(utils,mw,{cloneFunctions: true});
		mw.gui = Cu.cloneInto(managerFuncs,mw,{cloneFunctions: true});
		mw.addEventListener("beforeunload", onManagerBeforeUnload, true);
		mw.addEventListener("unload", onManagerUnload, true);
		var mworker = tab.attach( { 'contentScriptFile' : [jq,bs] });
		if (winutils.isDocumentLoaded(mwin.content)) {
			onManagerLoad();
		}
		else {
...
function initPlayer(tab) {
	...
	wrapWin.som = Cu.cloneInto(som,wrapWin,{cloneFunctions: true});
	wrapWin.db = Cu.cloneInto(db,wrapWin,{cloneFunctions: true});
	wrapWin.utils = Cu.cloneInto(utils,wrapWin,{cloneFunctions: true});
	wrapWin.gui = Cu.cloneInto(playerFuncs,wrapWin,{cloneFunctions: true});
...
function onPrefResetSom() {
	...
	var ret = env.alertResetToFactoryData();
	if (ret) {
		utils.log("reset to factory data");
		onPrefStopSom();
		...
		utils.log("reset to factory data cancled");
	} 
}
/**
...
/**
 * eventHandler bound to the manager dom window (mw) beforeunload event. 
 *   
 * 
 * @private
 * @method onManagerBeforeUnload
 **/  
function onManagerBeforeUnload(e) {
	utils.log("beforeunload: " + e);
	//e.preventDefault();
}
...
function getManagerTemplates() {
	for each (var tmp in Object.keys(mts)) {
		//utils.log("load: " + tmp);
		getManagerTemplate(tmp);
	}
}

function getManagerTemplate(tmp) {
